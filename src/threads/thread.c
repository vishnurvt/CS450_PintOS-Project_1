...
/* Start own code */
/* create priority_greater to compare priorities */
/* Checks if an element is present in a list. */
void donation_list_yield(void);
void priority_reload(void);
...
thread_unblock (struct thread *t) 
{
  ...
  if (intr_context () == false && t->priority > thread_get_priority())
    thread_yield();
  /* End own code */
}
...
thread_set_priority (int new_priority) 
{
  ...
  /* Reload the priority based on donations, if any. */
  priority_reload();

  /* If the new priority is lower, yield the CPU to a higher-priority thread. */
  if (new_priority < old_priority)
  {
    donation_list_yield();  // Yield if there's a higher-priority thread
  }
...
{
/*Start own code*/ /*editing get priority*/
  enum intr_level old_level = intr_disable();
               ...
               thread_func *function, void *aux) 
{
  ...
  int count = 0;
  thread_foreach(thread_count, &count);
  printf("number of threads: %d\n", count);

  struct aux_lowest_priority lowest = {PRI_MAX, "default"};
  thread_foreach(thread_store_priority, &lowest);
...
void thread_count(struct thread *t, void *aux){
    if (strcmp((*t).name, "idle") != 0){
      int *count = aux;
      (*count)++;
    }
}
void thread_store_priority(struct thread *t, void *aux) {
  struct aux_lowest_priority *lowest = aux;
  if (strcmp((*t).name, "idle") != 0){
    if ((*t).priority < (*lowest).priority){
        (*lowest).priority = (*t).priority;
        (*lowest).name = (*t).name;
    }
  }
}
...
void priority_reload(void) 
{
  ...
  if (!list_empty(&cur->donation_list)) {
    struct thread *highest_donor = list_entry(list_front(&cur->donation_list), struct thread, donation_list_elem);
...
void remove_with_lock(struct lock *lock) 
{
  ...
  for (e = list_begin(&thread_current()->donation_list); e != list_end(&thread_current()->donation_list); e = list_next(e)) {
    struct thread *t = list_entry(e, struct thread, donation_list_elem);
...
void donation_list_yield(void) {
    ...
    // Check if the current thread's priority is lower than the highest priority in the ready list
    if (!list_empty(&ready_list)) {
        ...
        if (next_thread->priority > current_thread->priority) {
            thread_yield(); // Yield if there's a higher-priority thread
        }
    }
}
